...
function getMaxValue(goods, space) {
    ...
    for (var i = 0; i < l; i++) {
        for (var s = 0; s <= space; s++) {
            if (i === 0) {
                ...
                var good = goods[i];
                var vOfRemainSpace = good.weight <= s ? vTable[i - 1][s - good.weight] : 0;
                if (good.weight <= s) {
                    var vWithGoodInSpace = good.value + vOfRemainSpace;
